# --------------------------------------------------
# Lesson 11 Build 1: Local Function Calling Simulator
# Microsoft Generative AI for Beginners adaptation
# Purpose: Understand function calling without needing Azure/OpenAI API access.
# --------------------------------------------------


def search_research_records(topic="", crop="", method="", context=""):
    """
    Simulates searching a research database.
    """
...
def generate_research_question(topic="", crop="", context=""):
    """
    Generates a structured research question from inputs.
    """
...
def draft_deadline_reminder(project="", deadline="", next_action=""):
    """
    Drafts a simple workflow message.
    """

    return {
        "subject": f"Deadline Reminder: {project}",
        "message": f"Hello, this is a reminder that '{project}' has an upcoming deadline on {deadline}. Next action required: {next_action}. Please review and update the project status."
    }


def detect_function(user_message):
    """
    Detect which function should be called based on user intent.
    More specific intents are checked before broad search intents.
    In a real function-calling app, the LLM would do this.
    """

    text = user_message.lower()

    # 1. Research question / title generation should be checked first
    if (
    ...
    # 2. Reminder / workflow message intent
    if (
        "reminder" in text
        or "deadline" in text
        or "email" in text
...
def extract_arguments(user_message):
    """
    Simple keyword-based argument extraction.
    This is intentionally basic for learning.
    """
...
print("Lesson 11: Function Calling")
...
while True:
    ...
    if user_message.lower().strip() in ["exit", "quit", "stop"]:
        print("Function calling simulator ended.")
        break
